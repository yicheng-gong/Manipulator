...
class YoubotTrajectoryPlanning(object):
    ...
    def q6(self):
        ...
        sorted_checkpoint_idx = list(sorted_order)
        full_checkpoint_tfs = self.intermediate_tfs(sorted_checkpoint_idx, target_cart_tf, 4)
        self.publish_traj_tfs(full_checkpoint_tfs)

        init_joint_position = target_joint_positions[:, 4]
    ...
    def load_targets(self):
        ...
        bags_messages = bag.read_messages("joint_data")
        index = 1
        for topic, jointState, t in bags_messages:
            target_joint_positions[:, index] = jointState.position
            target_cart_tf[:, :, index] = self.kdl_youbot.forward_kinematics(target_joint_positions[:, index])
            index = index + 1
    ...
    def get_shortest_path(self, checkpoints_tf):
        ...
        sorted_order = np.zeros(5).T
        position = np.c_[checkpoints_tf[:3, 3, 0], checkpoints_tf[:3, 3, 1], 
                   checkpoints_tf[:3, 3, 2], checkpoints_tf[:3, 3, 3], checkpoints_tf[:3, 3, 4]]
        index = [1, 2, 3, 4]
        tmp = itertools.permutations(index)
        permut = []
        for res in tmp:
            permut.append(list(res))
        permut = np.array(permut)
        permut = np.c_[np.zeros([permut.shape[0],1], dtype=int),permut]
        dist = permut[:, 0:1]
        for i in range(permut.shape[1] - 1):
            dist = dist + np.sqrt(np.sum((position[:, permut[:, i]].T - position[:, permut[:,i+1]].T)**2, axis=1))
        sorted_order = permut[np.argmin(dist), :]
    ...
    def intermediate_tfs(self, sorted_checkpoint_idx, target_checkpoint_tfs, num_points):
        ...
        for i in range(4):
            index1 = sorted_checkpoint_idx.index(i)
            index2 = sorted_checkpoint_idx.index(i+1)
            full_checkpoint_tfs[:, :, (i + 1) * (num_points + 1)] = target_checkpoint_tfs[:, :, index2]
            tfs = self.decoupled_rot_and_trans(target_checkpoint_tfs[:, :, index1], target_checkpoint_tfs[:, :, index2], num_points)
            full_checkpoint_tfs[:, :, i * (num_points + 1) + 1 : (i + 1) * (num_points + 1)] = tfs
    ...
    def decoupled_rot_and_trans(self, checkpoint_a_tf, checkpoint_b_tf, num_points):
        ...
        p_a = checkpoint_a_tf[:3, 3:4]
        p_b = checkpoint_b_tf[:3, 3:4]
        r_a = checkpoint_a_tf[:3, :3]
        r_b = checkpoint_b_tf[:3, :3]
        for i in range(num_points):
            p = (p_a + t * (i + 1) * (p_b - p_a))
            r = r_a + t * (i + 1) * expm(logm(r_a.T @ r_b) * t * (i + 1))
            tfs[:, :, i] = np.r_[np.c_[r, p], checkpoint_a_tf[3:4, :]]
    ...
    def full_checkpoints_to_joints(self, full_checkpoint_tfs, init_joint_position):
        ...
        for i in range(n):
            ...
            while True:
                error_pre = error
                q, error = self.ik_position_only(pose, q0)
                # if np.linalg.norm(q - q0) < 1e-10:
                if error < 1e-8:
                    qi.append(list(q))
                    q0 = q.copy()
                    break
                    ...
                    q0 = q.copy()
                    if abs(error_pre - error) < 1e-8:
                        qi.append(list(q))
                        break
        q_checkpoints = np.array(qi).T
    ...
    def ik_position_only(self, pose, q0):
        ...
        q0 = q0.reshape(1,-1)[-1, :]
        p0 = np.array(self.kdl_youbot.forward_kinematics(q0))[:3, 3]
        jac = self.kdl_youbot.get_jacobian(q0)[:3, :]
        p = pose[:3, 3]
        e = (p - p0).reshape(-1, 1)
        q = q0.copy().reshape(-1, 1)
        q = np.array(q + jac.T @ e).reshape(-1,1)[:, -1]
